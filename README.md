...
- `/resume [query]`: list past conversations, optionally filtered by search term.
- `/undo`: undo the last file patch applied in this session.
...
A workspace can have a `.jasmine/workspace.toml` for workspace-specific settings:

```toml
# Trust specific command prefixes (no approval prompt)
trusted_prefixes = ["git status", "git diff", "rg"]

# Auto-stash uncommitted changes before file patches (git only)
git_safety = true
```

When `git_safety` is enabled, Jasmine creates a `git stash` before modifying
files, so uncommitted work is never lost.
...
├── config.py         # ProfileConfig: unified provider + workspace config
├── tools.py          # ToolRegistry + built-in tool implementations
...
├── _html_extract.py  # Structured HTML extraction (web_extract backend)
├── _markdown.py      # Compact Markdown renderer (theme + classes)
