...
class OpenAIBackend:
    ...
    def __init__(
        ...
        stream_env = (os.environ.get("JASMINE_STREAM") or os.environ.get("AI_CODE_STREAM")) or "1"
        ...
        if enable_thinking is None:
            thinking_env = os.environ.get("JASMINE_THINKING") or os.environ.get("AI_CODE_THINKING") or None
    ...
    def _normalize_messages(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for message in messages:
            ...
            if role == "tool":
                ...
                continue
            # Skip unknown roles instead of silently defaulting to "user"
        return self._sanitize_orphaned_tool_calls(normalized)

    def _sanitize_orphaned_tool_calls(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Strip tool_calls from assistant messages whose tool responses are missing.

        When a turn is interrupted after the model emits tool_calls but before the
        agent executes them, the conversation history can contain an assistant
        message with tool_calls that is NOT followed by matching tool messages.
        Providers reject this with a 400 error.  We detect the mismatch and remove
        the orphaned tool_calls so the API call succeeds.
        """
        i = 0
        while i < len(messages):
            ...
            if msg.get("role") == "assistant" and msg.get("tool_calls"):
                ...
                # Collect the tool messages immediately following this assistant msg
                j = i + 1
                ...
                if found_ids != expected_ids:
                    # Orphaned: strip tool_calls to avoid API 400 errors
                    msg = dict(msg)
                    msg.pop("tool_calls", None)
                    messages[i] = msg
                i = j  # skip past the tool messages we already checked
            else:
                i += 1
        return messages
